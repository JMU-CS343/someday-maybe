...
import {
  ...
  export function renderBoards(root) {
    ...
    addTile.innerHTML = `<button class="add-list" title="Add new list">＋</button>`;
  ...
  function renderList(list) {
    ...
    <button class="menu list-menu" type="button" title="List menu">⋯</button>
  </div>
  
      <div class="tasks"></div>
      <div class="add-wrap"><button class="add-task">＋ Add task</button></div>
    `;
      ...
      `, (act) => {
        if (act === 'rename') {
          ...
          if (confirm('Delete this list and all its tasks?')) {
            deleteList(list.id); node.remove();
          }
        }
      });
    });
